...
def obj_cent(traj_nam):
  q1, q2, q3, q4, q5 = [(0.4, 0.4), (-0.4, 0.4), (-0.4, -0.4), (0.4, -0.4), (0.0, 0.0)]
...
class ewc_reg(Regularizer):
    ...
    def __call__(self, x):
        regularization = 0.
        regularization += self.Lambda * K.mean(self.fisher * K.square(x - self.prior_weights))
...
#   output1 = Dense(1, activation='linear', name='VALUE1')(dense2)
...
#   flatencoder = Flatten()(encoder)
#   #flatencoder = GlobalAveragePooling2D()(encoder)
#   dense0 = Dense(nodes, activation = 'sigmoid', name='LEC', kernel_regularizer=ewc_reg(I[2], old_wts[2], Lambda = lam),
...
#   dense_pi1 = Dense(nodes, activation= 'sigmoid', name='MEC', kernel_regularizer=ewc_reg(I[4], old_wts[4], Lambda = lam),
...
#   conv_layer0 = Flatten()(conv_layer)
#   dense1 = Dense(nodes2, activation=activ, name = 'D1', kernel_regularizer=ewc_reg(I[-6], old_wts[-6], Lambda = lam),
#                  bias_regularizer = ewc_reg(I[-5], old_wts[-5], Lambda = lam))(conv_layer0)
#   dense2 = Dense(nodes, activation=activ, name = 'D2', kernel_regularizer=ewc_reg(I[-4], old_wts[-4], Lambda = lam),
#                  bias_regularizer = ewc_reg(I[-3], old_wts[-3], Lambda = lam))(dense1)
...
#   output1 = Dense(1, activation='linear', name='VALUE1', kernel_regularizer=ewc_reg(I[-2], old_wts[-2], Lambda = lam),
#                   bias_regularizer = ewc_reg(I[-1], old_wts[-1], Lambda = lam))(dense2)
#   # output2 = Dense(1, activation='linear', name='VALUE2')(conv_layer[:,1,:])
#   #
...
def model_arch_catas(data, old_mod_dict, nodes, nodes2, lam, activ, ):
  ...
  encoder = Conv2D(4, (5, 5), padding='same', activation= act, name="CONV_1", kernel_regularizer=ewc_reg(I[0], old_wts[0], Lambda = lam),
                   bias_regularizer = ewc_reg(I[1], old_wts[1], Lambda = lam))(input_im43g)
  encoder = MaxPooling2D(pool_size=(8,8), padding='same', name="MAXPOOL_1")(encoder)
  # encoder = Conv2D(4, (5, 5), padding='same', activation= act, name="CONV_2", kernel_regularizer=ewc_reg(I[2], old_wts[2], Lambda = lam),
                  #  bias_regularizer = ewc_reg(I[3], old_wts[3], Lambda = lam))(encoder)
  # encoder = MaxPooling2D(pool_size=(2,2), padding='same', name="MAXPOOL_2")(encoder)
  ...
  # conv_layer0 = layers.Add()([conv_layer[:,0,:],conv_layer[:,1,:]])
  conv_layer0 = Flatten()(conv_layer)
  dense1 = Dense(nodes2, activation=activ, name = 'D1', kernel_regularizer=ewc_reg(I[-8], old_wts[-8], Lambda = lam),
                 bias_regularizer = ewc_reg(I[-7], old_wts[-7], Lambda = lam))(conv_layer0)
  dense2 = Dense(nodes2, activation=activ, name = 'D2', kernel_regularizer=ewc_reg(I[-6], old_wts[-6], Lambda = lam),
                 bias_regularizer = ewc_reg(I[-5], old_wts[-5], Lambda = lam))(dense1)
  dense3 = Dense(nodes2, activation=activ, name = 'D3', kernel_regularizer=ewc_reg(I[-4], old_wts[-4], Lambda = lam),
                 bias_regularizer = ewc_reg(I[-3], old_wts[-3], Lambda = lam))(dense2)
...
#   conv_layer0 = Flatten()(conv_layer)
#   #
#   repres_model = Model([input_img, input_pi], conv_layer0)
#   #regressor_model = Model([input_img,], [outputx, outputy, outputz])
#   opt = tf.keras.optimizers.Adam(learning_rate= lr)
#   repres_model.compile(optimizer=opt, loss="mse", )
#   # regressor_model.summary()

#   return repres_model


# def val_mod(data, nodes2, activ,):
#   input_dat = Input(shape = data.shape[1:])

#   dense1 = Dense(nodes2, activation = activ, name = 'D1')(input_dat)
#   # dense1 = BatchNormalization()(dense1)
#   # dense2 = Dense(nodes, activation = activ, name = 'D2')(dense1)
#   # dense3 = Dense(nodes, activation = activ, name = 'D3')(dense2)

#   output1 = Dense(1, activation='sigmoid', name='VALUE1')(dense1)

#   value_model = Model(input_dat, output1)

#   return value_model


# def val_mod_catas(data, old_mod_dict, nodes2, lam, activ, ):

#   old_wts = old_mod_dict["weights"]
#   I = old_mod_dict["fisher"]

#   input_dat = Input(shape = data.shape[1:])
#   dense1 = Dense(nodes2, activation=activ, name = 'D1', kernel_regularizer=ewc_reg(I[0], old_wts[0], Lambda = lam),
#                  bias_regularizer = ewc_reg(I[1], old_wts[1], Lambda = lam))(input_dat)
#   # dense1 = BatchNormalization()(dense1)
#   # dense2 = Dense(nodes, activation=activ, name = 'D2', kernel_regularizer=ewc_reg(I[2], old_wts[2], Lambda = lam),
#                  # bias_regularizer = ewc_reg(I[3], old_wts[3], Lambda = lam))(dense1)
#   # dense3 = Dense(nodes, activation=activ, name = 'D3', kernel_regularizer=ewc_reg(I[4], old_wts[4], Lambda = lam),
#                  # bias_regularizer = ewc_reg(I[5], old_wts[5], Lambda = lam))(dense2)
#   # dense4 = Dense(50, activation=act, name = 'D4')(dense3)

#   output1 = Dense(1, activation='sigmoid', name='VALUE1', kernel_regularizer=ewc_reg(I[-2], old_wts[-2], Lambda = lam),
#                   bias_regularizer = ewc_reg(I[-1], old_wts[-1], Lambda = lam))(dense1)
#   # output2 = Dense(1, activation='linear', name='VALUE2')(conv_layer[:,1,:])
#   #
#   model_r = Model(input_dat, output1)
